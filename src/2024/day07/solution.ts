...
function part1(input: string): number {
  ...
  return getTotalCalibrationResult(calibrationEquations)
}
...
function part2(input: string): number {
  const calibrationEquations = parse(input)
  return getTotalCalibrationResult(calibrationEquations, true)
}
...
function getTotalCalibrationResult(
  ...
  for (const { remainingNumbers, testValue } of calibrationEquations) {
    ...
    function dfs(
      ...
      const nextRemainingNumbersIndex = currentRemainingNumbersIndex + 1

      const canEquationPossiblyBeTrue =
        // prettier-ignore
        dfs(nextRemainingNumbersIndex, currentResult + currentRemainingNumber) ||
        dfs(nextRemainingNumbersIndex, currentResult * currentRemainingNumber) ||
        (hasConcatenation &&
          dfs(nextRemainingNumbersIndex, concatenation(currentResult, currentRemainingNumber))
        )

      memo.set(memoKey, canEquationPossiblyBeTrue)
      return canEquationPossiblyBeTrue
    }
...
function concatenation(number1: number, number2: number): number {
  return Number(`${number1.toString()}${number2.toString()}`)
}
